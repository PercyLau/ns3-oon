...
#include <boost/iostreams/stream.hpp>
  ...
  {
    mpdMetaDataFilename = m_mpdDirectory+"/"+mpdMetaDataFilename;
    NS_LOG_FUNCTION(mpdMetaDataFilename );
  ...
  */
  ...
  DASHFakeServerApplication::StartApplication (void)
  {
    ...
    for (std::vector<std::string>::iterator it = metaDataRepresentations.begin(); it != metaDataRepresentations.end(); ++it)
    {
      ...
      SSMpdFilename << m_mpdDirectory <<"BBB-2s.mpd.gz";
...
bool
DASHFakeServerApplication::CompressString(std::string input, std::stringstream& outputStream)
{
